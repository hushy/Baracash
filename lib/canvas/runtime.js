...
(function ()
{
	...
	cnds.IsAlphaLessThan = function (x, y,radius, alpha)
	{
		var ctx = this.ctx;
		var data = ctx.getImageData(x-radius/2, y-radius/2, radius, radius).data;
		var a = 0;
		for(var i = 0;i < radius;i++){
			a+=data[Math.floor(i+Math.random(0.9)*radius)*4+3]; //alpha offset
		}
		a /= radius;
		return a <= alpha;
	};	
	...
	acts.DrawLine = function (x1, y1, x2, y2, color, line_width)
	{
		...
		acts.ClearLine = function (x1, y1, x2, y2, color, line_width)
	{	
		var ctx = this.ctx;	
	...
	acts.clearPath = function (x,y,xTo,yTo,color,width){
	
	}
	
	acts.clearCircle = function (x, y, radius, color, line_width)
	{
		var ctx = this.ctx;
		ctx.globalCompositeOperation = "destination-out";
		ctx.strokeStyle = color;
